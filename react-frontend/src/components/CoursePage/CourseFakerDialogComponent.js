...
import courseFakerFactory from "./courseFakerFactory";
...
const CourseFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = courseFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("course").create(elem));
...
export default connect(null, mapDispatch)(CourseFakerDialogComponent);
